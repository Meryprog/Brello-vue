<template>
    <div v-for="category in categories" :key="category.id" class="flex flex-col flex-shrink-0 w-72">
			<div class="bg-white rounded flex items-center flex-shrink-0 h-10 px-2">
				...
				<button @click="test" title="Add Post" class="flex items-center justify-center w-6 h-6 ml-auto text-indigo-500 rounded hover:bg-indigo-500 hover:text-indigo-100" id="menu-button" aria-expanded="true" aria-haspopup="true">
					<svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
						<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path>
					</svg>
				</button>
				<button @click="deleteCard(category.id)" title="Delete Card" class="flex items-center justify-center w-6 h-6 ml-auto text-indigo-500 rounded hover:bg-red-500 hover:text-white-100">
			...
			<div class="flex flex-col pb-2 overflow-auto overscroll-none scroller">
